...
import { KeyAuthorization, MultisigConfig, MultisigOperation, Transaction as ox_Transaction, SignatureEnvelope, TxEnvelopeTempo, } from 'ox/tempo';
 ...
 */
export function handleRequest(next, parameters) {
...
async function submit(options) {
    ...
    const serializedUnsigned = serializeUnsigned(envelope, options.serialized.startsWith(TxEnvelopeTempo.feePayerMagic));
    ...
    const incoming = signature.signatures.map((signature) => SignatureEnvelope.serialize(signature));
    ...
    const { final, transactionHash: initialTransactionHash } = await (async () => {
        try {
            ...
            const transactionHash = TxEnvelopeTempo.hash(TxEnvelopeTempo.deserialize(final));
...
async function approveKeyAuthorization(options) {
    ...
    const approval = await (async () => {
        if ('keyAuthorization' in value) {
            const authorization = (() => {
                try {
                    return KeyAuthorization.fromRpc(value.keyAuthorization);
                }
                catch {
                    throw new RpcResponse.InvalidParamsError({
            ...
            const keyAuthorization = KeyAuthorization.serialize(KeyAuthorization.from(unsigned));
            const config = MultisigConfig.from(signature.config);
            ...
            return {
                account: signature.account,
                approvals: signature.signatures.map((signature) => SignatureEnvelope.serialize(signature)),
                config,
                hash,
                keyAuthorization,
            };
        ...
        if ('hash' in value && 'signature' in value) {
            ...
            const signature = (() => {
                try {
                    return SignatureEnvelope.serialize(SignatureEnvelope.from(value.signature));
                }
                catch {
                    throw new RpcResponse.InvalidParamsError({
    ...
    const operation = await OperationStore.update(options.store, approval.hash, async (existing) => {
        ...
        return MultisigOperation.from({
            ...pending,
            keyAuthorization: MultisigOperation.serializeKeyAuthorization(approval.keyAuthorization, {
                account: approval.account,
...
function mergeTransaction(existing, incoming) {
    if (!existing)
        return incoming;
    const existingTransaction = TxEnvelopeTempo.deserialize(existing);
    const incomingTransaction = TxEnvelopeTempo.deserialize(incoming);
...
async function toTransaction(client, operation, store) {
    ...
    const current = operation.status === 'pending'
        ...
        : operation;
    const serialized = MultisigOperation.serializeTransaction(approvals.approvals.length > 0 ? current : operation, {
...
function serializeUnsigned(envelope, feePayer) {
    return TxEnvelopeTempo.serialize(envelope, feePayer
        ? envelope.from
