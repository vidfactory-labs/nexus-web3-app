...
const client_2 = require("@x402/evm/batch-settlement/client");
const client_3 = require("@x402/evm/exact/client");
const client_4 = require("@x402/evm/exact/v1/client");
const client_5 = require("@x402/evm/upto/client");
const builder_code_1 = require("@x402/extensions/builder-code");
const client_6 = require("@x402/svm/exact/client");
const client_7 = require("@x402/svm/exact/v1/client");
const account_signers_js_1 = require("./account-signers.js");
const builder_code_js_1 = require("./builder-code.js");
const constants_js_1 = require("./constants.js");
const cdp_js_1 = require("../client/cdp.js");
const apply_js_1 = require("./guardrails/apply.js");
const normalize_js_1 = require("./guardrails/normalize.js");
const smart_account_js_1 = require("./smart-account.js");
const DEFAULT_ACCOUNT_NAME = "x402-client-wallet-1";
const resolveWalletType = (type) => {
    if (!type)
        return "eoa";
    if (type === "eoa" || type === "smart")
        return type;
    throw new Error(`Unsupported wallet type "${type}". Supported values: "eoa", "smart".`);
};
const resolveAccountName = (config) => config?.accountName ?? DEFAULT_ACCOUNT_NAME;
/**
...
const resolveEnvironment = (config) => {
    const rawEnv = config?.environment ??
        process.env.CDP_X402_CLIENT_ENVIRONMENT;
    return rawEnv === "development" ? "development" : "production";
};
const buildEvmRpcUrlsByChainId = (rpcUrlsByCaip2) => {
    const result = {};
...
const setupCdpSigners = async (client, config) => {
    ...
    let evmSigner;
    if (walletType === "smart") {
        const ownerAccountName = walletConfigInput?.type === "smart" ? walletConfigInput.ownerAccountName : undefined;
        if (!ownerAccountName) {
            throw new Error('Missing required owner account name for wallet type "smart". ' +
        ...
        evmSigner = (0, account_signers_js_1.fromCdpSmartWallet)(smartAccount);
    }
    else {
        const evmAccount = await cdpClient.evm.getOrCreateAccount({ name: accountName });
        evmAddress = evmAccount.address;
        evmSigner = (0, account_signers_js_1.fromCdpEvmAccount)(evmAccount);
    }
    const svmSigner = (0, account_signers_js_1.cdpSolanaAccountToSvmSigner)(svmAccount);
    // Keyed by CAIP-2 identifier (e.g. "eip155:8453"), not by network name.
    const defaultEvmRpcUrls = await (0, constants_js_1.getDefaultEvmRpcUrls)();
    const defaultBaseRpcUrl = defaultEvmRpcUrls[constants_js_1.baseMainnetCaip2]?.rpcUrl;
    const defaultBaseSepoliaRpcUrl = defaultEvmRpcUrls[constants_js_1.baseSepoliaCaip2]?.rpcUrl;
    ...
    const defaultNetworkSchemes = environment === "development"
        ? [
            {
    ...
    const evmRpcUrlsByCaip2 = {};
    for (const [network, netConfig] of networksByName) {
        if (network === "solana" || network === "solana-devnet")
            continue;
        if (netConfig.rpcUrl)
            evmRpcUrlsByCaip2[(0, normalize_js_1.normalizeNetwork)(network)] = { rpcUrl: netConfig.rpcUrl };
    }
    const evmRpcUrlsByChainId = buildEvmRpcUrlsByChainId(evmRpcUrlsByCaip2);
    for (const [network, netConfig] of networksByName) {
        const caip2Network = (0, normalize_js_1.normalizeNetwork)(network);
        const isSolana = network === "solana" || network === "solana-devnet";
        if (netConfig.scheme.exact) {
            if (isSolana) {
                client.register(caip2Network, new client_6.ExactSvmScheme(svmSigner, { rpcUrl: netConfig.rpcUrl }));
                client.registerV1(network, new client_7.ExactSvmSchemeV1(svmSigner, { rpcUrl: netConfig.rpcUrl }));
            }
            else {
                client.register(caip2Network, new client_3.ExactEvmScheme(evmSigner, evmRpcUrlsByChainId));
                client.registerV1(network, new client_4.ExactEvmSchemeV1(evmSigner));
            }
        }
        ...
        if (netConfig.scheme.upto && walletType !== "smart") {
            if (isSolana) {
                ...
                client.register(caip2Network, new client_5.UptoEvmScheme(evmSigner, evmRpcUrlsByChainId));
            }
        }
        if (netConfig.scheme.batchSettlement) {
            if (isSolana) {
                ...
                client.register(caip2Network, new client_2.BatchSettlementEvmScheme(evmSigner, { rpcUrl: netConfig.rpcUrl }));
            }
        }
    }
    if (config?.spendControls) {
        (0, apply_js_1.applySpendControls)(client, config.spendControls);
    }
 ...
 * ```
 */
class CdpX402Client extends client_1.x402Client {
    _config;
    _serviceBuilderCodes;
    _initPromise = null;
    _addresses = null;
    /**
    ...
    constructor(config) {
        ...
        this._serviceBuilderCodes = [
            ...(config?.builderCode !== undefined ? (0, builder_code_js_1.toServiceBuilderCodes)(config.builderCode) : []),
            builder_code_js_1.CDP_SDK_CLIENT_BUILDER_CODE,
        ];
        /*
         ...
         */
        this.registerExtension(new builder_code_1.BuilderCodeClientExtension(this._serviceBuilderCodes));
        this._config = config;
    }
    /**
...
exports.CdpX402Client = CdpX402Client;
